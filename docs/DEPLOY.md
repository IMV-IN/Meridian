...
```bash
docker pull ghcr.io/imv-in/meridian:0.9.0
...
| Path | Content |
|------|---------|
| `logging.jsonl_path` | Request metadata JSONL (no prompts by default) |
| `budgets.sqlite_path` | Budget counters |
| `cost.sqlite_path` | Cost ledger (enterprise: **sqlite**, not memory) |

Backup with the same cadence as other DB files. Cost ledger uses SQLite WAL — include `-wal`/`-shm` or checkpoint first.
...
Gateway is mostly I/O bound; size backends/GPUs separately. Revisit after a short soak on your models.

## Enterprise cost

See [`ENTERPRISE_COST.md`](./ENTERPRISE_COST.md) before enabling `cost:`.

...
- **Retry with exponential backoff** (`resilience.max_retries`, default 0 = off) — retries transport errors (`httpx.RequestError`: no response received) with `retry_backoff_base * 2^n` backoff, but only in provably safe windows: non-stream requests and the **open phase of streams** (before the first byte — a started stream never retries). Retried attempts counted in `meridian_upstream_retries_total{backend}`.
- **Streams always end well-formed** — any upstream transport failure (open phase or mid-stream) now yields an SSE error event + `data: [DONE]` instead of silently breaking (mid-stream stalls are accounted as 504 `stream_read_timeout`, other transport errors as 502).
...
- **Architecture diagram** — detailed L7 gateway diagram at [`docs/assets/architecture_diagram.png`](docs/assets/architecture_diagram.png) (linked from README + docs index).
